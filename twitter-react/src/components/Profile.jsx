    ...
    <div className="min-vh-100 col-sm-9 col-9 col-md-10 col-lg-6">
      <div className="content-container h-50 p-0">
        <div className="bgTweeter bg-height position-relative">
          <img className="profileImageHero" alt="imgHero" src={profile.avatar} />
        </div>
        {user.username === params.username ? (
          <form className="d-flex" action="/usuarios/profileUser.id/follow" method="post">
            <button disabled className="btn text-white rounded-pill ms-auto mt-3 mb-3">
              Follow
            </button>
          </form>
        ) : (
          <div className="d-flex">
            <button className="btn bgTweeter rounded-pill ms-auto mt-3 mb-3">Follow</button>
          </div>
        )}
        ...
        <div className="d-flex justify-content-end">
          ...
          <Link to={`/${profile.username}/following`} className="ms-auto text-decoration-none text-muted">
            Following
          </Link>
          <Link to={`/${profile.username}/followers`} className="ms-3 text-decoration-none text-muted">
            Followers
          </Link>
        </div>
        ...
        <div className="border-top p-3">
          {user.username === params.username
            ? tweets.map((tweet) => {
                return <ProfileTweet key={tweet.id} profile={profile} tweet={tweet} getTweets={getTweets} />;
              })
            : clickedUserTweets.map((tweet) => {
                return <ProfileTweet key={tweet.id} profile={profile} tweet={tweet} getClickedUserTweets={getClickedUserTweets} />;
              })}
        </div>
      </div>
    </div>
